...
- 功能：深色星空 Canvas 背景、阅读进度环、导航/目录滚动虚化、目录平滑滚动、全文搜索（Ctrl K）。站点定位是**纯资料储藏室**：无搜索发现面板、无每日知识点、无闯关游戏、无错题本（2026-09-06 用户指令陆续移除，组件都在 git 历史 backup-full-features 标签里）。
